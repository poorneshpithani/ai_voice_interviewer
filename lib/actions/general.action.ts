...
export async function createFeedback(params: CreateFeedbackParams) {
  ...
  - Confidence & Clarity.`,
      ...
      if (!result || !result.object) {
        console.error("Failed to generate feedback object:", result);
        return { success: false };
      }
  ...
  export async function getFeedbackByInterviewId(params: GetFeedbackByInterviewIdParams): Promise<Feedback | null > {
    ...
    // const feedback = await db.collection('feedback').where('finalized', '==', interviewId).where('userId', '==', userId).limit(1).get()

    const feedback = await db
  .collection('feedback')
  .where('interviewId', '==', interviewId) // ✅ Correct field
  .where('userId', '==', userId)
  .limit(1)
  .get()
